var ind_flag_arr_str =$("#IndustryID").val();
if(ind_flag_arr_str==""||ind_flag_arr_str==null){
	var ind_flag_arr = new Array();  // 已选中数组
}else{
	var ind_flag_arr = new Array();
	var ind_flag_arr_strs=ind_flag_arr_str.split(',');
	$.each(ind_flag_arr_strs,function(idx,v){
		ind_flag_arr[idx]=v;
	});
	var funStr='';
	for(var i in ind_flag_arr){
		funStr+=','+ind_a[ind_flag_arr[i]];
	}
	funStr=funStr.substring(1)?funStr.substring(1):'请选择行业';
	$('#btn_IndustryID').val(funStr);
}
